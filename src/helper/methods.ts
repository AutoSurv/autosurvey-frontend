...
export const downloadExcel = (
  data: ExportedSurvey[], // changed from any
  ...
  const worksheet = XLSX.utils.json_to_sheet(data as ExportedSurvey[], {
    header: myHeader,
  });
  /*   delete(worksheet['W1']);
  delete(worksheet['X1']);
  ...
  let fileName = organization.orgName;
  console.log(fileName)

  if (data.length === 1) {
    fileName += `${survey.year}_${survey.country}_${survey.locationClustered}_${survey.id}.xlsx`;
    // XLSX.writeFile(
    //   workbook,
    ...
    if (filterCountries.length === 1) {
      fileName += `_${survey.country}`;
      console.log("here is", filterCountries);
      if (filterLocations.length === 1) {
        fileName += `_${survey.locationClustered}`;
        if (filterYears.length === 1) {
          fileName += `_${survey.year}.xlsx`;
        }
      }
    } else {
      fileName += ".xlsx";
  ...
  //       XLSX.writeFile(
  //         workbook,
  //         organization.orgName + "_" + survey.country + ".xlsx"
  //       );
  //     }
  //   } else {
...
export const offlineDownloadExcel = (
...
export const saveOfflineSurvey = (
  e: FormEvent<HTMLFormElement>,
  offlineSurveys: Survey[],
  offlineSurveyCounter: number,
  setSurveyCounter: Dispatch<SetStateAction<string>>
) => {
...
export async function signInJwtTokenHandler(
...
export async function signUpHandler(
  ...
  await signUpUser(inputSignUpBody).then((response) => {
...
export function checkImportedSurveyFields(data: ImportedSurvey[]) {
  data.forEach((survey) => {
    if (survey.country === null) {
      survey.country = "";
    }
    if (typeof survey.year !== "number") {
      survey.year = 0;
    }
    if (typeof survey.rent !== "number") {
      survey.rent = 0;
    }
    if (typeof survey.utilities !== "number") {
      survey.utilities = 0;
    }
    if (typeof survey.food !== "number") {
      survey.food = 0;
    }
    if (typeof survey.basicItems !== "number") {
      survey.basicItems = 0;
    }
    if (typeof survey.transportation !== "number") {
      survey.transportation = 0;
    }
    if (typeof survey.educationTotal !== "number") {
      survey.educationTotal = 0;
    }
    if (typeof survey.educationSupplies !== "number") {
      survey.educationSupplies = 0;
    }
    if (typeof survey.educationFee !== "number") {
      survey.educationFee = 0;
    }
    if (survey.educationType == null) {
      survey.educationType = "";
    }
    if (survey.accommodationType === null) {
      survey.accommodationType = "";
    }
    if (survey.profession === null) {
      survey.profession = "";
    }
    if (survey.locationGiven === null) {
      survey.locationGiven = "";
    }
    if (survey.locationClustered == null) {
      survey.locationClustered = "";
    }
    if (typeof survey.numResidents !== "number") {
      survey.numResidents = 0;
    }
    if (typeof survey.numIncomes !== "number") {
      survey.numIncomes = 0;
    }
    if (typeof survey.numFullIncomes !== "number") {
      survey.numFullIncomes = 0;
    }
    if (typeof survey.numChildren !== "number") {
      survey.numChildren = 0;
    }
    if (typeof survey.totalIncome !== "number") {
      survey.totalIncome = 0;
    }
...
export function calculateMeanValues(
  ...
  for (let fiveVarIndex = 0; fiveVarIndex < fiveVar.length; fiveVarIndex++) {
    ...
    for (
      let countryIndex = 0;
      countryIndex < country_arr.length;
      countryIndex++
    ) {
      const lowerPart = filteredSurvey
        .filter((s) => {
          if (prop && isSurveyKey(prop, s)) {
            return (
              s[prop] &&
              (country_arr[countryIndex] === s.locationClustered ||
                country_arr[countryIndex] === s.country)
            );
          }
        })
        .map((s) => {
...
export function handleClickedCountry(
  geoName: string,
  clickedCountries: ClickedCountry[],
  setClickedCountries: Dispatch<SetStateAction<ClickedCountry[]>>
) {
  if (
    clickedCountries.filter((c) => c.country === geoName && c.country !== "")
      .length > 0
  ) {
